...
@section{Exercise 2.97}

@chunk[<reduce-poly>
(define (reduce-terms n d)
  (let* ((gp (gcd-terms n d))
         (nn (car (div-terms n gp)))
         (dd (car (div-terms d gp)))
         (cl (append (all-coeffs nn) (all-coeffs dd)))
         (gl (foldl gcd (car cl) (cdr cl))))
    (list (reduce-coeffs nn gl) (reduce-coeffs dd gl))))
(define (reduce-poly p1 p2)
  (if (same-variable? (variable p1) (variable p2))
      (let ((rt (reduce-terms (term-list p1) (term-list p2))))
        (map make-poly (list (variable p1) (variable p2)) rt))
      (list p1 p2)))
(put 'reduce '(polynomial polynomial)
     (lambda (p1 p2) (map tag (reduce-poly p1 p2))))
]

@chunk[<reduce-integers>
...
@section{Exercise 2.96}

@chunk[<psuedoremainder-terms>
(define (pseudoremainder-terms a b)
  (let* ((o1 (order (first-term a)))
         (o2 (order (first-term b)))
         (c (coeff (first-term b)))
         (f (expt c (sub (add 1 o1) o2)))
         (int-factor (make-sparse-termlist (list (list 0 f)))))
    (cadr (div-terms (mul-terms int-factor a) b))))

(define (all-coeffs tl)
  (define (iter cl tls)
    (if (empty-termlist? tls)
        cl
        (iter (cons (coeff (first-term tls)) cl)
              (rest-terms tls))))
  (iter '() tl))
(define (gcd-coeffs a)
  (let* ((cl (all-coeffs a)))
    (foldl gcd (car cl) (cdr cl))))

(define (reduce-coeffs tl gcd)
  (if (empty-termlist? tl)
      (the-empty-termlist tl)
      (adjoin-term (make-term (order (first-term tl))
                              (div (coeff (first-term tl)) gcd))
                   (reduce-coeffs (rest-terms tl) gcd))))
]

@section{Exercise 2.94}

@chunk[<install-gcd-poly>
<psuedoremainder-terms>

(define (gcd-terms a b)
  (if (empty-termlist? b)
      (reduce-coeffs a (gcd-coeffs a))
...
(test-suite
 ...
 ; exercise 2.95
 (let* ((p1 (make-polynomial-sparse 'x '((2 1) (1 -2) (0 1))))
        (p2 (make-polynomial-sparse 'x '((2 11) (0 7))))
        (p3 (make-polynomial-sparse 'x '((1 13) (0 5))))
        (q1 (mul p1 p2))
        (q2 (mul p1 p3)))
 ...
 ; exercise 2.97
 (let* ((p1 (make-polynomial-sparse 'x '((1 1) (0 1))))
        (p2 (make-polynomial-sparse 'x '((3 1) (0 -1))))
        (p3 (make-polynomial-sparse 'x '((1 1))))
        (p4 (make-polynomial-sparse 'x '((2 1) (0 -1))))
...
(test-suite
 ...
 (let* ((p1 (make-polynomial-sparse 'x '((2 1) (0 1))))
   ...
   (check-equal? '(rational (polynomial x sparse (3 2) (0 2))
