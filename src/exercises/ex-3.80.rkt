...
;; 3-36.  If R, L, and C are the resistance, inductance, and
;; capacitance, then the relations between voltage (v) and current (i)
...
;; should produce a procedure that takes the initial values of the
;; state variables, v_(C_0) and i_(L_0), and produces a pair (using
;; `cons') of the streams of states v_C and i_L.
...
(define (RLC R L C dt)
  (define (circuit vc0 iL0)
    ...
    (list-of-streams vc iL))
  circuit)

;; Using `RLC', generate the pair of streams that models the behavior
;; of a series RLC circuit with R = 1 ohm, C = 0.2 farad, L = 1 henry,
;; dt = 0.1 second, and initial values i_(L_0) = 0 amps and v_(C_0) =
;; 10 volts.

(define RLC1 (RLC 1 1 0.2 0.1))
(define RLC1-stream (RLC1 10 0))
