...
We need to make primitive scheme numbers work without modification or
tagging. The main change is to the data accessors, of which we add to
each a clause to check whether the datum is a basic native scheme
number.
...
We also change the definition of the tag function within the
scheme-number package so that it does not add any tag at all.

@chunk[<scheme-number-tag>
(define (tag x) x)
]

Some basic tests confirm that now scheme-numbers are identical to
native scheme-numbers.

@chunk[<tests-untagged-scheme-number>
