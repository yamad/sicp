...
The goal is to define a generic predicate to test if the number equals
zero.

This exercise is almost identical in structure to the previous
exercise. First, we define the generic operation

@chunk[<zero?-generic-operation>
(define (=zero? a) (apply-generic '=zero? a))
]

Then, we install the type specific versions into each package

@chunk[<zero?-scheme-number>
