...
;; A_1, A_2, A_3, ..., A_n and B_1, B_2, B_3, ..., B_n are the two
;; binary numbers to be added (each A_k and B_k is a 0 or a 1).  The
;; circuit generates S_1, S_2, S_3, ..., S_n, the n bits of the sum,
;; and C, the carry from the addition.  Write a procedure
...
;; propagate.  What is the delay needed to obtain the complete output
;; from an n-bit ripple-carry adder, expressed in terms of the delays
...
(define (ripple-carry-adder a-list b-list c-in s-list c-out)
  (define (next-adder a b s c)
    (let ((c-out (make-wire)))
      (full-adder a b c s c-out)
      c-out))
  (foldl next-adder a-list b-list s-list c-in)
  (set-signal! c-out 0))
